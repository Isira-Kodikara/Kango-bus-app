...
ALTER TABLE routes ADD COLUMN osm_id BIGINT UNIQUE;
ALTER TABLE bus_stops ADD COLUMN osm_node_id BIGINT UNIQUE;

-- Route segments table (stores travel data between consecutive stops)
...
-- Journey plans table (stores user journey requests)
CREATE TABLE IF NOT EXISTS journey_plans (
    ...
    -- FOREIGN KEY (user_id) REFERENCES users(user_id), -- Uncomment if users table exists and has user_id
    INDEX idx_user (user_id),
    INDEX idx_created (created_at)
);

-- Seed Route Segments Data (Run ONLY after routes and route_stops are populated)
...
WHERE NOT EXISTS (
    SELECT 1 FROM route_segments WHERE route_id = rs.route_id AND from_stop_id = rs.stop_id
);
